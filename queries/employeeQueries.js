...
async function addEmployee(firstName, lastName, departmentId, roleId, managerId, salary) {
  try {
    ...
    // Set manager_id to NULL if not provided
    const managerIdValue = managerId === '' ? null : managerId;

    await pool.query(query, [firstName, lastName, departmentId, roleId, managerIdValue, salary]);
  } catch (error) {
    throw error;
  }
}
...
async function removeEmployee(employeeId) {
  try {
    ...
    const [deleteResult] = await pool.query(query, [employeeId]);
